...
def fetch_real_news_sentiment(ticker=None):
    ...
    try:
        # Search for news about each ETF (or specific ticker)
        tickers_to_search = [ticker] if ticker else ETF_LIST
        
        for t in tickers_to_search:
            # 1. Try to get ETF news directly
            try:
                stock = yf.Ticker(t)
                news_items = []
                ...
                if news_items and len(news_items) > 0:
                    news_found = True
                    for item in news_items[:2]:  # Get top 2 ETF articles
                        ...
                        if t not in sentiment_scores:
                            sentiment_scores[t] = sentiment
            except Exception as e:
                pass
            
            # 2. Search underlying stocks
            underlying_stocks = ETF_INFO[t]["top_holdings"]
            for stock_ticker in underlying_stocks[:3]:  # Top 3 underlying stocks
                try:
                    stock = yf.Ticker(stock_ticker)
                    if hasattr(stock, 'news') and stock.news:
                        news_found = True
                        item = stock.news[0]  # Get most recent
                        title = item.get('title', 'No title')
                        publisher = item.get('publisher', 'Unknown')
                        link = item.get('link', '')
                        ...
                        articles.append({
                            ...
                            "source": publisher,
                            "time": "Recent",
                            "summary": f"News about {stock_ticker}, a top holding in {t}: {title[:150]}...",
                            "link": link
                        })
                        
                        # Average sentiment for the ETF
                        if t in sentiment_scores:
                            sentiment_scores[t] = (sentiment_scores[t] + sentiment) / 2
                        else:
                            sentiment_scores[t] = sentiment
                except Exception as e:
                    pass
            
            # 3. Search connected market/index
            ...
            if underlying_index in index_ticker_map:
                ...
                try:
                    ...
                    if hasattr(index, 'news') and index.news:
                        news_found = True
                        item = index.news[0]  # Get most recent
                        title = item.get('title', 'No title')
                        publisher = item.get('publisher', 'Unknown')
                        link = item.get('link', '')
                        ...
                        articles.append({
                            ...
                            "source": publisher,
                            "time": "Recent",
                            "summary": f"Market news for {underlying_index}, which {t} tracks: {title[:150]}...",
                            "link": link
                        })
            ...
            # If still no sentiment for this ticker, set neutral
            if t not in sentiment_scores:
                ...
                articles.append({
                    ...
                    "source": "System",
                    "time": "N/A",
                    "summary": f"No news feeds available for {t}, its underlying stocks, or {underlying_index}. Using price trends for analysis."
                })
        ...
        st.warning(f"Could not fetch live news, using simulated data. Error: {str(e)}")
        return simulate_news_sentiment()
...
def analyze_sentiment_from_title(title):
    ...
    positive_words = ['surge', 'gain', 'rally', 'jump', 'soar', 'climb', 'rise', 'beat', 'strong', 'growth', 
                     'profit', 'bullish', 'upgrade', 'outperform', 'record', 'high', 'boost', 'positive']
    negative_words = ['fall', 'drop', 'decline', 'plunge', 'crash', 'loss', 'weak', 'bearish', 'downgrade',
                     'miss', 'cut', 'concern', 'worry', 'risk', 'threat', 'negative', 'low', 'sink']
    ...
    return net_score
    """Simulate news sentiment analysis (would use real web search in production)"""
    # This simulates what the real web search would return
    sentiment_scores = {}
    articles = []
    
    for ticker in ETF_LIST:
        # Simulate sentiment (-1 to 1)
        base_sentiment = np.random.uniform(-0.3, 0.7)  # Slightly positive bias
        ...
        if sentiment > 0.3:
            ...
            article_class = "sentiment-positive"
            titles = [
                f"{ticker}: Strong Performance Expected",
                f"Analysts Bullish on {ticker} Strategy",
                f"{ticker} Benefits from Market Conditions"
            ]
        elif sentiment < -0.3:
            article_sentiment = "NEGATIVE"
            article_class = "sentiment-negative"
            titles = [
                f"{ticker}: Concerns Raised About Sustainability",
                f"Volatility Impacts {ticker} Returns",
                f"{ticker} Faces Headwinds"
            ]
        else:
            article_sentiment = "NEUTRAL"
            article_class = "sentiment-neutral"
            titles = [
                f"{ticker}: Steady Performance Continues",
                f"Mixed Signals for {ticker}",
                f"{ticker} Maintains Course"
            ]
        ...
        articles.append({
            ...
            "sentiment_class": article_class,
            "sentiment_score": sentiment,
            "source": np.random.choice(["MarketWatch", "Bloomberg", "CNBC", "Reuters"]),
            "time": f"{np.random.randint(1, 12)} hours ago",
            "summary": f"Recent market analysis suggests {article_sentiment.lower()} outlook for {ticker}. "
                      f"{'Positive momentum continues.' if sentiment > 0 else 'Monitoring situation closely.' if sentiment > -0.3 else 'Caution advised.'}"
        })
...
# =====================================================
# TAB 2: WEEKLY INVESTMENT ADVISOR (NEW!)
# =====================================================
with tab2:
...
# Continue with remaining tabs (3-8) - keeping them as before but can add more enhancements...
# For brevity, I'll add the key new tabs and you can keep the others from the previous version

# =====================================================
# TAB 5: SAFETY MONITOR (Enhanced)
# =====================================================
with tab4:
    if not st.session_state.PORTFOLIO_LOCKED:
...
# =====================================================
# TAB 6: NEWS & INTELLIGENCE (Enhanced with Real-time)
# =====================================================
with tab5:
    if not st.session_state.PORTFOLIO_LOCKED:
        ...
        if selected_etf != "ALL":
            articles_to_show = [a for a in articles_to_show if a["ticker"] == selected_etf]
        ...
        # In production, this would fetch real news for each underlying stock
        st.markdown("**Note:** In production, this section would display real-time news for each underlying stock (AAPL, MSFT, NVDA, etc.) using live web search.")

# Keep remaining tabs (3, 4, 7, 8) from previous version...
# I'll add placeholders to complete the structure
